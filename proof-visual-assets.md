# Proof Systems Website Visual Assets

## Positioning

Use recreated, anonymised workflow visuals as the default. Real screenshots should only be used after a separate approval pass confirms they contain no client, staff, project, financial, credential or commercially sensitive data.

## Brand and type

Canonical logo, icon and favicon files live in `assets/brand/` and were copied from `50_assets/proof-systems/brand/current/`. Inter Variable 4.1 is self-hosted in `assets/fonts/` under the SIL Open Font License 1.1. No remote font or CDN is used.
...
| `assets/demo-media/cpr-demo.mp4` and `cpr-poster.jpg` | Central Project Register story film | Staged AGE-600 synthetic master | Public-safe synthetic media; hash verified; publication still requires Mat's separate approval |
...
The original `synthetic-inbox-to-handover.png` asset contains only generic interface labels and invented shapes. It includes no names, email addresses, companies, project references, financial data, credentials, metadata from a client source or recognisable software branding.

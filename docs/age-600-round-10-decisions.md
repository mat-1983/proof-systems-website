...
| 2026-08-31 | Superseded for enquiry routing only: Clear workflow now uses `workflow.html?interest=focused-build` and Needs clarity uses `workflow.html?interest=workflow-diagnostic`. Team capability remains an informational `training.html` link. |

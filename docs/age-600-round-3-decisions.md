...
| 2026-08-29 | The base `.gap-scene` two-column rule overrode the earlier 900px one-column declaration, so the Gap WebP sat in one grid track (~623px at 1488px). A later `#gap .gap-scene` / `#gap .approved-visual` desktop block now spans the full wrap. |
| 2026-08-29 | `.approach-banner { width: 100%; }` overrode `.desktop-hide-copy { width: 1px; }` at desktop, expanding `p.approach-banner.desktop-hide-copy` to ~1489px and making document scrollWidth 1642 at 1488px. A later 900px utility with compound selectors and `!important` now keeps hidden copy as a 1px clipped box. |

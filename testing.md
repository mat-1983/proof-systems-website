...
| 04 / Approach | 1,237 px | 900 px | 275 px → 202 px | 27.3% |
...
- PASS — `git diff --check`.
- NOT COMPLETE — live 1440 desktop plus 768 tablet and 390/360/320 forward and reverse scroll inspection. Chrome headless in this environment cannot create a unique user-data directory and is blocked from writing Crashpad state.
...
- PASS — `python3 site_check.py` including `check_approach_stage_scope`. That assertion fails on `f0fb01c` CSS (4 failures) and passes on the corrected tree.
- PASS — `python3 proof_media_check.py`.
- PASS — `python3 privacy_notice_check.py`.
- PASS — `python3 crawl_check.py`.
- PASS — `node --check` on `assets/js/site.js` and `assets/js/form.js`.
- PASS — in-memory `ast.parse` of the four checkers.
- PASS — `git diff --check`.
- NOT COMPLETE — live 1024px contrast of Approach titles on sand. Chrome headless previously SIGSEGV in this environment.
...
- PASS — `python3 site_check.py` including `check_round8`.
- PASS — `python3 proof_media_check.py`.
- PASS — `python3 privacy_notice_check.py`.
- PASS — `python3 crawl_check.py`.
- PASS — `node --check` on `assets/js/site.js` and `assets/js/form.js`.
- PASS — in-memory `ast.parse` of the four checkers.
- PASS — `git diff --check`.
- NOT COMPLETE — live layout at 320–1440, staged forward/reverse scroll, reduced-motion, no-JavaScript and overflow. Chrome headless previously SIGSEGV in this environment.

## AGE-600 round 7 review correction — Selected Systems hash offset — 31 August 2026

`#individual-systems`, `#connected-workflow` and `#finance` use `scroll-margin-top: calc(var(--nav-h) + 0.5rem)` so direct hash arrival clears the sticky desktop navigation.

- PASS — `python3 site_check.py` including the Selected Systems `scroll-margin-top` / `--nav-h` assertion.
- PASS — `python3 proof_media_check.py`.
- PASS — `python3 privacy_notice_check.py`.
- PASS — `python3 crawl_check.py`.
- PASS — `node --check` on `assets/js/site.js` and `assets/js/form.js`.
- PASS — in-memory `ast.parse` of the four checkers.
- PASS — `git diff --check`.
- NOT COMPLETE — live 1280/1024 hash-arrival measurement of the first card heading versus the sticky nav. Chrome headless previously SIGSEGV in this environment.
